...
public class MusicListAdapter extends BaseAdapter{
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if(position == pos) {
			viewHolder.albumImage.setImageResource(R.mipmap.item);
		} else {
			Bitmap bitmap = Constant.getArtwork(context, mp3Info.getId(),mp3Info.getAlbumId(), true, true);
			if(bitmap == null) {
				viewHolder.albumImage.setImageResource(R.mipmap.icon);
			} else {
		...
		viewHolder.musicDuration.setText(Constant.formatTime(mp3Info.getDuration()));//显示时长
